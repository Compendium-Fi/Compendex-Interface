...
export function SwapTokenContextProvider(props: any) {
    ...
    useEffect(() => {
        ...
        if (
            ...
            // Fetch SPL tokens.
            getOwnedAssociatedTokenAccounts(connection, publicKey).then((accs) => {
                if (accs) {
                    // @ts-ignore
    ...
    useEffect(() => {
        ...
        if (
            ...
            if (tokenAccount && isSol) {
                listener = connection.onAccountChange(publicKey, (info) => {
                    const token = {
                    ...
                    (info) => {
                        if (info.data.length !== 0) {
                            try {
                                ...
                                if (token.amount !== tokenAccount.account.amount) {
                                    const index =
                                        _OWNED_TOKEN_ACCOUNTS_CACHE.indexOf(tokenAccount);
                                    assert.ok(index >= 0);
                                    _OWNED_TOKEN_ACCOUNTS_CACHE[index].account = token;
                                    setRefresh((r) => r + 1);
                                }
