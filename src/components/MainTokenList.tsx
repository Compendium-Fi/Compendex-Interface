...
export function MainTokenListContextProvider(props: any) {
  ...
  const swappableTokens = useMemo(() => {
    let newList = tokenList.map((t: any) => {
      if (t.symbol == "LARIX") {
        let t1 = t;
        t1.extensions.serumV3Usdc =
    ...
    const tokens = newList.filter((t: TokenInfo) => {
      const isUsdxQuoted =
        t.extensions?.serumV3Usdt || t.extensions?.serumV3Usdc;
      return isUsdxQuoted;
    });
    const filterTokens = tokens.filter(
      (tkn: TokenInfo) => tkn.symbol !== "usdt" && tkn.symbol !== "usdc"
    );
    filterTokens.sort((a: TokenInfo, b: TokenInfo) =>
